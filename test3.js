...
What is the longest distance that they can possibly create between each other, if they also choose to sit on the optimal starting block initially?
...
2. Given blocks = [1, 5, 5, 2, 6], the function should return 4. If starting from blocks[3], the first frog can jump to blocks[1], but not to blocks[0], and the second frog can jump to blocks[4].
...
function solution(blocks) {
    ...
    let maxDistance = 0;
    let maxBlockHeight = blocks[0];
    let currentDistance = 0;
  
    for (let i = 1; i < N; i++) {
      if (blocks[i] >= maxBlockHeight) {
        maxBlockHeight = blocks[i];
        maxDistance = Math.max(maxDistance, i - currentDistance);
        currentDistance = i;
      }
    }
